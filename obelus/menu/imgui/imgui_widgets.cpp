...
void ui::TabButton3(const char* label, const char* label2, int* selected, int num, int total) {
    ...
    {
        if (g_menu.baszom_anyadat_budos_cigany_fasszopo_roman_stheoknak_meg_cat_vagy_ki_a_geci_nek_soha_nem_lesz_skeetje == 2)
            pos = window->Pos + ImVec2(10 - 8 - 46 + 2 + 230 + 12 + 10 - 15 - 5 - 17 - 10 - 10 - 100 - 54 + 12 + 41, 9 + 24 - 20 + 30) + ImVec2((window->Size.x + 200 - 12 - 12 - 90 - 30 - 100 + 5 - 40 + 10 - 4 - 5 + 2 + 25 + 15 + 25 + 15 + 5) / total * num, 0) - ImVec2(0, 1);
        else
    ...
    if (*selected == num)
        textColor = ImColor(config.menu_color[0], config.menu_color[1], config.menu_color[2], g.Style.Alpha);
    ...
    if (total == 1)
    {
        ...
        if (g_menu.baszom_anyadat_budos_cigany_fasszopo_roman_stheoknak_meg_cat_vagy_ki_a_geci_nek_soha_nem_lesz_skeetje == 3)
            window->DrawList->AddText(ImVec2(pos.x + 7 - 4 + size.x / 2 - label_size.x / 2 + 9 + 3, pos.y + size.y / 2 - label_size.y / 2 - 1 + 21 - 3), textColor, label2);
        else
    ...
    {
        if (g_menu.baszom_anyadat_budos_cigany_fasszopo_roman_stheoknak_meg_cat_vagy_ki_a_geci_nek_soha_nem_lesz_skeetje == 2)
            window->DrawList->AddText(ImVec2(pos.x + 8 - 4 + size.x / 2 - label_size.x / 2, pos.y + size.y / 2 - label_size.y / 2 - 1 - 3), textColor, label);
        else
            window->DrawList->AddText(ImVec2(pos.x - 4 + size.x / 2 - label_size.x / 2, pos.y + size.y / 2 - label_size.y / 2 - 1 - 3), textColor, label);
        ui::PushFont(ui::GetIO().Fonts->Fonts[2]);

        if (g_menu.baszom_anyadat_budos_cigany_fasszopo_roman_stheoknak_meg_cat_vagy_ki_a_geci_nek_soha_nem_lesz_skeetje == 2)
            window->DrawList->AddText(ImVec2(pos.x + 8 - 4 + size.x / 2 - label_size.x / 2 + 9 + 3 + 10, pos.y + size.y / 2 - label_size.y / 2 - 1 + 21 - 3), textColor, label2);
        else
            window->DrawList->AddText(ImVec2(pos.x - 4 + size.x / 2 - label_size.x / 2 + 9 + 3, pos.y + size.y / 2 - label_size.y / 2 - 1 + 21 - 3), textColor, label2);
    }
...
bool ui::Checkbox(const char* label, bool* v, bool safe)
{
    ...
    if (*v)
        window->DrawList->AddRectFilledMultiColor(pos + ImVec2(2, 2), pos + ImVec2(10, 10), ImColor(config.menu_color[0], config.menu_color[1], config.menu_color[2], g.Style.Alpha), ImColor(config.menu_color[0], config.menu_color[1], config.menu_color[2], g.Style.Alpha), ImColor(30 / 255.f, 30 / 255.f, 30 / 255.f, g.Style.Alpha), ImColor(30 / 255.f, 30 / 255.f, 30 / 255.f, g.Style.Alpha));
        //window->DrawList->AddRectFilled(pos + ImVec2(2, 2), pos + ImVec2(10, 10), ImColor(config.menu_color[0], config.menu_color[1], config.menu_color[2], g.Style.Alpha));
    else {
        if (IsItemHovered())
...
bool ui::MultiSelect(const char* label, std::unordered_map<int, bool>* data, std::vector<const char*> items) {
    ...
    if (ui::BeginCombo(label, (howmuchsel(*data) > 0 ? std::to_string(howmuchsel(*data)) + " ..." : "none").c_str(), 0, items.size())) {
        for (int i = 0; i < items.size(); i++)
...
bool ui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, int remove_from_fmt)
{
    ...
    window->DrawList->AddRectFilledMultiColor(slider_bb.Min, ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)), slider_bb.Max.y), ImColor(config.menu_color[0], config.menu_color[1], config.menu_color[2], g.Style.Alpha), ImColor(config.menu_color[0], config.menu_color[1], config.menu_color[2], g.Style.Alpha), ImColor(30 / 255.f, 30 / 255.f, 30 / 255.f, g.Style.Alpha), ImColor(30 / 255.f, 30 / 255.f, 30 / 255.f, g.Style.Alpha));
...
bool ui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    window->DrawList->PushClipRect(bb.Min, bb.Max, true);
    window->DrawList->AddText(bb.Min + ImVec2(8, size.y / 2 - label_size.y / 2 + 2), selected ? ImColor(config.menu_color[0], config.menu_color[1], config.menu_color[2], g.Style.Alpha) : ImColor(180 / 255.f, 180 / 255.f, 180 / 255.f, g.Style.Alpha), label);
    window->DrawList->PopClipRect();
