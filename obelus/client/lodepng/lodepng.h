...
typedef struct LodePNGColorStats {
  ...
  /*pUser settings for computing/using the stats*/
...
buffer and *outsize its size in bytes. out must be freed by pUser after usage.
*/
...
[ ] allow pUser to provide custom color conversion functions, e.g. for premultiplied alpha, padding bits or not, ...
[ ] allow pUser to give data (void*) to custom allocator
...
simplicity, the following design choices were made: There are no client
...
If the input and output color type differ (whether pUser chosen or auto chosen),
