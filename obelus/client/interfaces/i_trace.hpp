...
struct ray_t {
	...
	__forceinline ray_t(const vec3_t& start, const vec3_t& end, const vec3_t& mins, const vec3_t& maxs) {
		...
		m_start_offset = VectorAligned{ mins + maxs };
		m_start_offset *= 0.5f;
		m_start = VectorAligned{ start + m_start_offset };
		m_start_offset *= -1.f;
	}
...
using ShouldHitFunc_t = bool(__cdecl*)(BaseEntity*, int);
class trace_filter_simple : public i_trace_filter {
public:
...
public:
	__forceinline trace_filter_simple() {}

	__forceinline trace_filter_simple(const BaseEntity* pass_ent1, int collision_group = 0, ShouldHitFunc_t shouldhit_check_fn = nullptr) :
		m_pass_ent1{ pass_ent1 },
		m_collision_group{ collision_group },
		m_shouldhit_check_fn{ shouldhit_check_fn } {}

	bool ShouldHitEntity(void* pEntityHandle, int contentsMask) {
		return (pEntityHandle == m_pass_ent1);
	}	
	TraceType_t GetTraceType() const {
		return TRACE_EVERYTHING;
	}
	virtual void SetPassEntity(BaseEntity* pass_ent1) {
		m_pass_ent1 = pass_ent1;
	}
...
public:
	...
	virtual int get_point_contents_collideable(Collideable* collide, const vec3_t& pos) = 0;
	virtual void clip_ray_to_entity(const ray_t& ray, unsigned int mask, BaseEntity* ent, trace_t* trace) = 0;
	virtual void clip_ray_to_collideable(const ray_t& ray, unsigned int mask, Collideable* collide, trace_t* trace) = 0;
