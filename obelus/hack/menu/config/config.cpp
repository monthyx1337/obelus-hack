...
void C::Export(std::string file) {

	std::filesystem::path fsFilePath(file);
	if (fsFilePath.extension() != (".cfg"))
		fsFilePath.replace_extension((".cfg"));
	...
	std::ifstream input_file = std::ifstream(szFile);
...
bool C::Import(std::string file) {
	...
	std::ofstream output = std::ofstream(szFile);
	if (!output.good())
		return false;
