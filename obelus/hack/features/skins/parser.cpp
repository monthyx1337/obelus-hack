...
std::vector<paintkits::paint_kit> paintkits::skin_kits;
std::vector<paintkits::paint_kit> paintkits::glove_kits;
std::vector<paintkits::paint_kit> paintkits::sticker_kits;
...
auto paintkits::initialize() -> void
{
	...
	const auto sig_address = pattern::Scan(XOR("client.dll"), XOR("E8 ?? ?? ?? ?? FF 76 0C 8D 48 04 E8"));
	...
	// Dump paint kits
	{
		...
		for (auto i = 0; i <= map_head->last_element; ++i)
		{
			...
			if (paint_kit->id < 10000)
				paintkits::skin_kits.push_back({ paint_kit->id, name });
			else
				paintkits::glove_kits.push_back({ paint_kit->id, name });
		}

		std::sort(paintkits::skin_kits.begin(), paintkits::skin_kits.end());
		std::sort(paintkits::glove_kits.begin(), paintkits::glove_kits.end());
	}
	...
	/*{
		...
		for (auto i = 0; i <= map_head->last_element; ++i)
		{
			...
			char name[256];
			V_UCS2ToUTF8(wide_name, name, sizeof(name));
			paintkits::sticker_kits.push_back({ sticker_kit->id, name });
		}
		std::sort(paintkits::sticker_kits.begin(), paintkits::sticker_kits.end());
		paintkits::sticker_kits.insert(paintkits::sticker_kits.begin(), { 0, "None" });
	}*/
	ParseSkins();
}
...
int paintkits::ParseSkins()
{
		...
		if (child->ToObject())
		{
			for (auto weapon : weaponNames)
			{
				...
				if (pos != std::string::npos)
				{
					...
					skins::get().weaponSkins[weapon].insert(
	...
	for (auto child : PaintKitNames->children)
	{
		if (child->ToKeyValue())
		{
			...
			{
				skins::get().skinNames[key] = child->ToKeyValue()->Value.toString();
			}
		}
	}
		...
		if (rarity->ToObject()) {
			for (const auto& child : rarity->children) {
				if (child->ToKeyValue()) {
					...
					auto skinInfo = &skins::get().skinMap[paint_kit_name];
	...
	console::log("[setup] paintkits initialized!\n");
	return 0;
}